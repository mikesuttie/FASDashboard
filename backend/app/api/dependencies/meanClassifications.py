...
async def get_meanClassification_by_id_from_path(
    id: int = Path(..., ge=1),
    ...
    meanClassification = await meanClassifications_repo.meanClassification_by_id(id=id, requesting_user=current_user)
...
def check_meanClassification_modification_permissions(
    current_user: UserInDB = Depends(get_current_active_user),
    meanClassification: MeanClassificationInDB = Depends(get_meanClassification_by_id_from_path),
) -> None:
