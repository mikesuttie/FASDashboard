...
from app.api.dependencies.patients import check_patient_creation_permissions, check_patient_modification_permissions, check_patients_reading_permissions, get_patient_by_id_from_path
...
async def get_all_patients(patients_repo: PatientsRepository = Depends(get_repository(PatientsRepository)),) -> List[dict]:
    patients = await patients_repo.get_all_patients()    
    return patients
...
async def create_new_patient(
    ...
    created_patient = await patients_repo.create_patient(new_patient=new_patient)
