...
def landmarkConsensusSelection(initial3DLandmarksDLIB, eyeLandmarks3D, faceLandmarks3D, portraitLandmarks3D):
    ...
    # TODO Provide info about origin of landmark detection, at least for both eyes individually! In the simplest case: A list (landmark names) of suboptimal LM selection choices.
...
async def detectLandmarks(faceFiles: List[UploadFile] = File(...)):
    ...
    with open('portrait2Dlms.json', 'w') as file:
        json.dump(portraitLandmarks2D, file, indent=4, sort_keys=True,
    ...
    #return {"msg":"ALL GOOD!!"+str(faceLandmarks3D) + "=========" + str(missingFaceLandmarks3D) + " ************** "  + str(eyeLandmarks3D) + " ------------------ " + str(portraitLandmarks3D)}     
    #print(faceLandmarks3D)
    ...
    result = { "initialLM_DLIB" : initial3DLandmarksDLIB, "eyeLM" : eyeLandmarks3D, "faceLM" : faceLandmarks3D, "missingFaceLM" : missingFaceLandmarks3D, "portraitLM": initial3DLandmarksFromPortraitModel, "consensus" : consensusSelectionLandmarks, "suboptimalConsensusSelections" : suboptimalSelections, "missingConsensusLandmarks" : missingConsensusLandmarks}
